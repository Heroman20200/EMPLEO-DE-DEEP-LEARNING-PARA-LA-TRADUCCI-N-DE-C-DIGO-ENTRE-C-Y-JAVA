...
# Define la ruta al archivo .exe y la línea de código como argumento
ruta_al_exe = '..\\..\\Data\\normalizedCollector\\normalizedCollector.exe'

def normalizeCollector(codigo):
    # Ejecuta el .exe con el código como argumento y capturar la salida
    resultado = subprocess.run([ruta_al_exe, codigo], capture_output=True, text=True, encoding='utf-8', errors='replace')
    # Obtiene los datos originales que han sido normalizados y dividirlos por lineas
    salida_normalizada = resultado.stdout.strip()
    print(salida_normalizada)
    lineas = salida_normalizada.split('\n')
    
    # Crea colas para métodos, variables y literales
    metodos = Queue()
    variables = Queue()
    literales = Queue()
